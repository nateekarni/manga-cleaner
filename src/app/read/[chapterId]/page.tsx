    ...
    useEffect(() => {
        const loadChapter = async () => {
            ...
            try {
                ...
                const res = await fetch(`${API_URL}/chapter/${chapterId}?source=${source}`);
                if (res.ok) {
                    ...
                    if (json.manga_id) {
                        try {
                            const safeMangaId = encodeURIComponent(json.manga_id);
                            const safeSource = encodeURIComponent(source);
                            const hRes = await fetch(`${API_URL}/history/${safeMangaId}?source=${safeSource}`);

                            if (hRes.ok) {
                                const entry = await hRes.json();
                    ...
                    if (json.manga_id) {
                        const mangaRes = await fetch(`${API_URL}/manga/${json.manga_id}?source=${source}`);
                        if (mangaRes.ok) {
                ...
                <div className="flex items-center px-4 h-16 gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-white/60 text-xs truncate">
                            {(() => {
                                const activeCh = chapters.find(ch => ch.id === currentChapterId || decodeURIComponent(ch.id) === decodeURIComponent(currentChapterId));
                                const title = activeCh?.title || currentChapterId;
                                const match = title.match(/(\d+(\.\d+)?)/);
                                return match ? `ตอนที่ ${match[0]}` : title;
                            })()}
                        </p>
                    </div>
                </div>
            ...
            <div className={`fixed bottom-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-xl border-t border-white/10 transition-transform duration-300 pb-safe ${showUI ? 'translate-y-0' : 'translate-y-full'}`}>
                <div className="flex items-center justify-between px-4 h-16 max-w-2xl mx-auto w-full gap-2">
                    ...
                    <div className="flex-1 max-w-[200px]">
                        <select
                            value={chapters.find(ch => ch.id === currentChapterId || decodeURIComponent(ch.id) === decodeURIComponent(currentChapterId))?.id || currentChapterId}
